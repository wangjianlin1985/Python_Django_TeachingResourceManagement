...
function initUserInfoManageTool() {
	userInfo_manage_tool = {
		...
		search: function() {
			...
			queryParams["userState"] = $("#userState").val();
				...
				$.ajax({
					...
					success : function (userInfo, response, status) {
						...
						if (userInfo) { 
							...
							$("#userInfo_className_edit").val(userInfo.className);
							$("#userInfo_className_edit").validatebox({
								required : true,
							...
							$("#userInfo_userState_edit").val(userInfo.userState);
							$("#userInfo_userState_edit").validatebox({
								required : true,
								missingMessage : "请输入账号状态",
							});
						} else {
