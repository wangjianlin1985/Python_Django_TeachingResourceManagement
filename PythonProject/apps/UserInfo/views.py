...
class FrontAddView(BaseView):  # 前台用户添加
    ...
    def post(self, request):
        ...
        userInfo.className = request.POST.get('userInfo.className')
        ...
        import datetime
        userInfo.address = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        userInfo.regTime = request.POST.get('userInfo.regTime')
        userInfo.userState = '正常'
...
class FrontListView(BaseView):  # 前台用户查询列表
    ...
    def handle(self, request):
        ...
        userState = self.getStrParam(request, 'userState')
        # 然后条件组合查询过滤
        userInfos = UserInfo.objects.all()
        if user_name != '':
            userInfos = userInfos.filter(user_name__contains=user_name)
        if className != '':
            userInfos = userInfos.filter(className__contains=className)
        if name != '':
        ...
        if userState != '':
            userInfos = userInfos.filter(userState__contains=userState)
...
class UpdateView(BaseView):  # Ajax方式用户更新
    ...
    def post(self, request, user_name):
        ...
        userInfo.regTime = request.POST.get('userInfo.regTime')
        userInfo.userState = request.POST.get('userInfo.userState')
        userInfo.save()
        return JsonResponse({'success': True, 'message': '保存成功'})
