...
class FrontLoginView(View):
    ...
    def post(self,request):
        ...
        try:
            userObj = UserInfo.objects.get(user_name=username, password=password,userState='正常')
...
class LoginView(View):
    ...
    def post(self,request):
        ...
        if identify == 'admin':
            ...
            try:
                teacher = Teacher.objects.get(teacherNo=username, password=password)
                request.session['teacherNo'] = username
                data = {'msg': '登录成功', 'success': True}
...
class MainTeacherView(View):
    # 后台主界面
    def get(self,request):
        return render(request, 'main_teacher.html')
